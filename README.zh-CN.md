...
输入框的模型菜单会在独立子菜单中显示 OpenClaw 针对当前模型提供的推理强度。使用自定义模型新建对话时，首次发送前也会根据 Gateway 提供的当前 Agent 默认值显示该控件；会话创建后则以会话元数据为准。所选强度仅对当前会话持续生效；“思考”开关则在显式关闭与此前选择或可用的启用档位之间切换。
