...
The composer model menu opens reasoning effort in a dedicated submenu using only levels advertised by OpenClaw for the current model. This control is also available on a new custom-model chat before its first message, using the current agent defaults advertised by Gateway until the session is created. A selected level applies to the current session, and the Thinking toggle switches between explicit Off and the prior or an available enabled level.
