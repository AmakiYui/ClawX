...
The composer model menu opens reasoning effort in a dedicated submenu using only levels advertised by OpenClaw for the current model. A selected level applies to the current session, and the Thinking toggle switches between explicit Off and the prior or runtime-default level.
...
Custom-provider model rows also receive an explicit `contextWindow` (inferred from the model family, e.g. `gpt-5.x` → 272k), and rows saved by older versions are backfilled on startup, so OpenClaw can compact long sessions before they fail with "Context overflow" errors. Reasoning is disabled by default for custom providers. You can explicitly enable it in the provider settings and select the primary model's supported Low / Medium / High / Extra High levels; ClawX never infers those levels from the model name. The Gateway then advertises the configured levels to Chat. When you have no compaction config, ClawX seeds `agents.defaults.compaction.mode = "safeguard"` and `reserveTokensFloor = 50000`; rows or configs you authored yourself are never modified (except missing inferred fields may be backfilled).
